...
# Target directory and Zip file paths (relative to script location, going up one level to Darta)
...
if [ ! -f "$ZIP_FILE" ]; then
    ...
    echo -e "  2. Letakkan berkas tersebut sejajar dengan file proyek ${BOLD}Darta.xcodeproj${NC}."
...
if [ -d "$DERIVED_DATA_DIR" ]; then
    echo -e "  - Menghapus Build & Indexing cache untuk Darta..."
    rm -rf "$DERIVED_DATA_DIR"/Darta-*/Build
    rm -rf "$DERIVED_DATA_DIR"/Darta-*/Index.noindex
...
if command -v xcodebuild &> /dev/null; then
    ...
    xcodebuild -project "$PARENT_DIR/Darta.xcodeproj" -resolvePackageDependencies -quiet
    echo -e "  ${GREEN}✓ Package Dependencies selesai diselaraskan.${NC}"

    echo -e "${BLUE}▶ Melakukan Clean Build Folder di Xcode...${NC}"
    xcodebuild -project "$PARENT_DIR/Darta.xcodeproj" -alltargets clean -quiet
